use ear_algae::{traits::Ring, Vect};
use plyers::ply::*;
...
impl<V: Copy + PlyVertex> Mesh<V> {
    pub fn into_ply<E: PlyElementSet>(
        self,
        encoding: Encoding,
    ) -> Result<Ply<E>, ConsistencyError> {
        let mut ply = Ply::new();
        ...
        ply.header.elements.add({
            ...
            def.properties.add(PropertyDef::new(
                ...
                PlyPropList::<u8, u32>::PLY_PROP_TYPE,
            ));
            def
        });
        let verts = self
            ...
            .iter()
            .copied()
            .map(PlyVertex::into_ply_element)
            .collect();
        ...
        let faces = self
            ...
            .map(|&t| {
                ...
                element.set_property(
                    "vertex_index".to_owned(),
                    PlyPropList::<u8, _>::from(t.map(|x| x as u32)).into_prop(),
                );
                element
            })
            .collect();
...
impl<S: Ring + PlyPropery> PlyVertex for Vect<3, S> {
    fn element_def(name: String) -> ElementDef {
        ...
        def.properties
            .add(PropertyDef::new("x".to_owned(), S::PLY_PROP_TYPE));
        def.properties
            .add(PropertyDef::new("y".to_owned(), S::PLY_PROP_TYPE));
        def.properties
            .add(PropertyDef::new("z".to_owned(), S::PLY_PROP_TYPE));
