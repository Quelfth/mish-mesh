...
use crate::{
    vertex::{VertexData, VertexPosition},
    Mesh,
};

impl<V: VertexPosition> Mesh<V>
where
    <V as VertexData>::Scalar: Axis,
{
    pub fn merge_by_distance(self, distance: V::Scalar) -> Self {
        ...
        while !tree_verts.is_empty() {
            while !tree_verts.contains_key(&i) {
                i += 1
            }
            ...
            for NearestNeighbour { item: i, .. } in
                tree.within_unsorted::<SquaredEuclidean>(&v.pos().0, distance.pow(2))
            {
